...
def show_filters(s,sel:int):
    ...
    for y in range(len(filter_list)):
        ...
        if y==sel:
            ...
            s.addstr(y+offset_y+1,offset_x,info,c.color_pair(BK))
    s.addstr(offset_y+1+len(filter_list),offset_x,f"{'|Space:Toggle, Enter:Modify, Esc: Close':{w0+w1+w2+w3+4}}|",c.color_pair(BK)|bold_for_header())
    s.refresh()
...
def save_settings():
    ...
    last.write(str(cl.cur_theme_idx)+"\n")
    #last.write(f"\n{cl.cur_color1}\n{cl.cur_color_bk1}\n{cl.cur_color2}\n{cl.cur_color_bk2}\n{cl.cur_color_s}\n{cl.cur_color_bk_s}\n{cl.cur_color_s_bold}\n")
    last.close()
...
def show_one_line_header(card_win,pg,move_y):
    props_len=0
    ...
    for f in one_line_headers[pg].keys():
        ...
        if w==0:
            w=max_name_l
        card_win.addstr(move_y,pos_x,"|",c.color_pair(SEPARATOR_BK)|c.A_BOLD)
        pos_x+=1
        card_win.addstr(move_y,pos_x,f"{f:{w}}",c.color_pair(SEPARATOR_BK)|c.A_BOLD)
        #if sort_mode1==one_line_headers[pg][f][2] or sort_mode2==one_line_headers[pg][f][2]:
            #card_win.addstr(move_y,pos_x,f"{f:{w}}",c.color_pair(SEPARATOR_BK)|c.A_BOLD)
        #else:
            #card_win.addstr(move_y,pos_x,f,c.color_pair(SEPARATOR_BK))
        pos_x+=w

def show_list(card_win,search_win,results,move_y=0,list_len=LIST_LINES):
    global selected_mon_name
    selected_mon_name=""
    if move_y==0:
        card_win.bkgd(' ',c.color_pair(SEPARATOR_BK_ALT))
        card_win.erase()
    show_one_line_header(card_win, list_pg,move_y)
    for x in range(list_len):
        if x+list_mode_skip>=len(list_mode_mons):
            break
        current_mon=table[list_mode_mons[x+list_mode_skip]]
        card_win.move(x+1+move_y,0)
...
def react_to_key_edit_colors(s,search_win,ch,key,alt_ch,results,mon_name):
    global reloaded
    global in_str
    global sel,skip
    global format_length
    ...
    if key=="s":
        cl.cur_theme_idx+=1
        if cl.cur_theme_idx>=len(cl.color_themes):
            cl.cur_theme_idx=0
...
def enter_numeric_param(card_win,search_win):
    ...
    filter_list[filter_mode_sel]=fs.make_param_filter("Parameter",filters_mode_param_str[param_caption],0,0)
    ...
    try:
        ...
        mode=FILTERS
        return 0
    filter_list[filter_mode_sel]=fs.make_param_filter("Parameter",filters_mode_param_str[param_caption],int(min),0)
    show_filters(card_win,filter_mode_sel)
    ...
    try:
        ...
        mode=FILTERS
        return 0
    filter_list[filter_mode_sel]=fs.make_param_filter("Parameter",filters_mode_param_str[param_caption],int(min),int(max))
...
def react_to_key_select_list(card_win,search_win,ch,key,alt_ch,mon_name,op_list,idx,skip):
    global mode
    global reloaded
        ...
        if idx>0:
            ...
            if skip>0:
                ...
                if len(op_list)>MAX_LINES_LIST:
                    idx=MAX_LINES_LIST-1
                    skip=len(op_list)-MAX_LINES_LIST
                else:
                    idx=len(op_list)-1
    if key=="KEY_DOWN" :
        if len(op_list)>MAX_LINES_LIST:
...
def main(s):
    ...
    while True:
        ...
        if mode==FILTERS:
            show_filters(card_win,filter_mode_sel)
        if mode==SELECT_SORT1:
            show_select_list(card_win,"Sort field (1)",sort_mode_str,sort_mode_sel,0)
        if mode==SELECT_SORT2:
            show_select_list(card_win,"Sort field (2)",sort_mode_str,sort_mode_sel,0)
        if mode==LIST:
        ...
        if mode==ENTER_NUMERIC_PARAM:
            ...
            continue
        if mode==SELECT_FILTER_GROUP:
            show_select_list(card_win,filter_list[filter_mode_sel]["name"],filters_group_names,filters_group_sel,filters_group_skip)
        ...
        if mode==EDIT_COLORS:
            ...
            list_mode_sel=3
            list_mode_skip=0
            show_list(card_win,search_win,results,move_y=12,list_len=10)
            ec_offset=0
            if edit_colors_x==0:
                ec_offset=45
            else:
                ec_offset=5
            show_edit_colors_hint(card_win,ec_offset)
        if mode==SHOW_ALL:
        ...
        if mode==SELECT_PARAM:
            ...
            res,param_mode_sel,notused=react_to_key_select_list(card_win,search_win,ch,key,alt_ch,mon_name,list(param_mode_list.keys()),param_mode_sel,0)
            if res!=0:
                break
            continue
        if mode==CARD:
            ...
            continue
        if mode==EDIT_COLORS:
            res=react_to_key_edit_colors(s,search_win,ch,key,alt_ch,results,mon_name)
            if res!=0:
                break
            continue            
        if mode==SELECT_SORT1 or mode==SELECT_SORT2:
            #res=react_to_key_select_sort(ch,key,alt_ch,mon_name)
            res,sort_mode_sel,notused=react_to_key_select_list(card_win,search_win,ch,key,alt_ch,mon_name,sort_mode_str,sort_mode_sel,0)
            if res!=0:
                break
            continue
        if mode==SELECT_FILTER_GROUP:
            #res=react_to_key_select_sort(ch,key,alt_ch,mon_name)
            res,filters_group_sel,filters_group_skip=react_to_key_select_list(card_win,search_win,ch,key,alt_ch,mon_name,filters_group_names,filters_group_sel,filters_group_skip)
...
if __name__=="__main__":
    ...
    try:
        ...
        cl.cur_theme_idx=int(last.readline().strip())
        for x in range(len(ver_list)):
